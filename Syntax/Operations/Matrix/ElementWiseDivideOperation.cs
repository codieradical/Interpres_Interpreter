...
{
    public class ElementWiseDivideOperation : IBinaryOperation
    {
        public object Operate(AbstractSyntax left, AbstractSyntax right)
        {
            if (left.GetValue().IsArray() && right.GetValue().IsArray())
            {
                ...
                if (leftArray.Length != rightArray.Length)
                    throw new SyntaxException($"Can't element-wise divide matrices of differing sizes.");
                ...
                try
                {
                    ...
                    for (int i = 0; i < result.Length; i++)
                    {
                        if (result[i].IsNumeric() && rightArray[i].IsNumeric())
                            ...
                            throw new SyntaxException($"Can't element-wise divide a {left.GetValue().GetType()} by a {right.GetValue().GetType()}");
                    }
