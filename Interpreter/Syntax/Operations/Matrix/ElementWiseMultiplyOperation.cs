...
{
    public class ElementWiseMultiplyOperation : IBinaryOperation
    {
        public object Operate(AbstractSyntax left, AbstractSyntax right)
        {
            if (left.GetValue().IsArray() && right.GetValue().IsArray())
            {
                ...
                if (leftArray.Length != rightArray.Length)
                    throw new SyntaxException($"Can't element-wise multiply matrices of differing sizes.");
                ...
                try
                {
                    ...
                    for (int i = 0; i < result.Length; i++)
                    {
                        if (result[i].IsNumeric() && rightArray[i].IsNumeric())
                            ...
                            throw new SyntaxException($"Can't element-wise multiply a {left.GetValue().GetType()} by a {right.GetValue().GetType()}");
                    }
